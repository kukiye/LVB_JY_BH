package com.hhzt.iptv.lvb_x.rootmanager;
...
import com.hhzt.iptv.lvb_x.rootmanager.container.Command;
import com.hhzt.iptv.lvb_x.rootmanager.container.Result;
import com.hhzt.iptv.lvb_x.rootmanager.container.Result.ResultBuilder;
import com.hhzt.iptv.lvb_x.rootmanager.container.Shell;
import com.hhzt.iptv.lvb_x.rootmanager.exception.PermissionException;
import com.hhzt.iptv.lvb_x.rootmanager.utils.Remounter;
...
public class RootManager {
	...
	public boolean grantPermission() {
		if (!hasGivenPermission) {
			...
			if (lastPermissionCheck < 0 || System.currentTimeMillis() - lastPermissionCheck > Constants.PERMISSION_EXPIRE_TIME) {
				hasGivenPermission = accessRoot();
				lastPermissionCheck = System.currentTimeMillis();
			}
		}
	 ...
	 * 
	 * @param apkPath
	 *            the file path of APK, do not start with <I>"file://"</I>. For
	 *            example, <I>"/sdcard/Tech_test.apk"<I> is good. Please do NOT
	 ...
	 * 
	 * @param packageName
	 *            the app's package name you want to uninstall.
	 ...
	 * 
	 * @param apkPath
	 *            the source apk path of system app.
	 * @return The result of run command operation or uninstall operation.
	 */
	 ...
	 * 
	 * @param path
	 *            the path you want to remount
	 * @param mountType
	 *            the mount type, including, <i>"ro" means read only, "rw" means
	 *            read and write</i>
	 * @return the operation result.
	 */
	 ...
	 * 
	 * @param binaryName
	 *            the file name of binary, containing params if necessary.
	 * @return the operation result.
	 */
	 ...
	 * 
	 * @param path
	 *            the file path of binary, containing params if necessary.
	 * @return the operation result.
	 */
	 ...
	 * 
	 * @param path
	 *            the path with file name and extend name.
	 * @return the operation result.
	 */
	 ...
	 * 
	 * @param processName
	 *            the name of process. For user app, the process name is its
	 *            package name.
	 ...
	 * 
	 * @param processName
	 *            the name of this process. For user app, the process name is
	 *            its package name.
	...
	private boolean accessRoot() {
		...
		Command commandImpl = new Command("id") {
			...
			@Override
			public void onUpdate(int id, String message) {
				if (message != null && message.toLowerCase(Locale.CHINA).contains("uid=0")) {
					accessRoot = true;
				}
			}
