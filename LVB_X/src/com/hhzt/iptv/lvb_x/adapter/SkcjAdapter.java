...
public class SkcjAdapter extends BaseAdapter {

	private Context mContext;
	private ArrayList<Skcj> mData;

	public SkcjAdapter(Context context, ArrayList<Skcj> datas) {
		mContext = context;
		mData = datas;
	}
	...
	private void setValue(Holder holder, int position) {
		Skcj model = mData.get(position);
		holder.item1.setText(model.getPrisonName());
		holder.item2.setText(model.getName());
		holder.item3.setText(model.getCode());
		holder.item4.setText(model.getKxcj());
		holder.item5.setText(DateTimeUtil.toTime(model.getDate(),
				DateTimeUtil.DATE_FORMATE_YEAR_MONTH_DAY));
		holder.item6.setText(model.getJyjd());
		holder.item7.setText(model.getSfhz());
	}
