...
public class VerticalScrollTextView extends TextView {
	...
	private IScrollerSuccessCB mScrollCB;
	private int mWidth;
	private int mHeight;
	private int mHeightMode;
	private int mHeightSize;

	public VerticalScrollTextView(Context context, float speed, IScrollerSuccessCB scrollCB) {
		super(context);
	...
	public void resetText() {
		int realHeight = measureContentHeight((int) mWidth);

		// 如果实际行宽少于预定的宽度，减少行宽以使其内容横向居中
		int leftPadding = getCompoundPaddingLeft();
		int rightPadding = getCompoundPaddingRight();
		mWidth = Math.min(mWidth, (int) lineWidthMax + leftPadding + rightPadding);
		...
		mHeight += getCompoundPaddingTop() + getCompoundPaddingBottom();
	...
	protected void onDraw(Canvas canvas) {
		...
		for (LINE aContentList : contentList) {
			...
			for (int j = 0; j < aContentList.line.size(); j++) {
				...
				if (ob instanceof String) {
					canvas.drawText((String) ob, realDrawedWidth, height + aContentList.height - paint.getFontMetrics().descent - mStep, paint);
		...
		mStep = mStep + mSpeed;
		if (mStep >= (contentList.size() * (paint.getTextSize() + getLineSpacingDP()) - getHeight() + dip2px(context, 25))) {
			if (null != mScrollCB) {
				mScrollCB.scrolled(true);
			}
			return;
	...
	//
	// textBgColorPaint.setColor(((BackgroundColorSpan)
	// span).getBackgroundColor());
	// textBgColorPaint.setStyle(Style.FILL);
	// textBgColorRect.left = (int) realDrawedWidth;
	// int textHeight = (int) getTextSize();
	// textBgColorRect.top = (int) (height + aContentList.height - textHeight -
	...
	// lineSpacing - paint.getFontMetrics().descent);
	...
	public void setMText(CharSequence cs) {
		...
		ArrayList<SpanObject> isList = new ArrayList<VerticalScrollTextView.SpanObject>();
		useDefault = false;
